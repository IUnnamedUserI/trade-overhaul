...
public record TradePayload(int syncId, int slot, boolean isBuying, boolean sellWholeStack) {
	...
	public static TradePayload read(PacketByteBuf buf) {
		return new TradePayload(buf.readInt(), buf.readInt(), buf.readBoolean(), buf.readBoolean());
	}
}
